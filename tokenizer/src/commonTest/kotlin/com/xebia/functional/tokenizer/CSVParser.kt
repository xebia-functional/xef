 ...
 *
 * A port of [OpenCSV](https://github.com/cygri/opencsv/blob/master/src/main/java/com/opencsv/RFC4180Parser.java)
 */
class RFC4180Parser(val quotechar: Char = '"', val separator: Char = ',') {
    ...
    while (currentPosition < nextLine.length) {
      ...
      currentPosition = when {
        ...
        nextQuote == -1 || nextQuote > nextSeparator || nextQuote != currentPosition -> {
          ...
          add(
            if (fieldEnd >= nextLine.length) nextLine.substring(currentPosition) else nextLine.substring(
              currentPosition,
              fieldEnd
            )
          )
  ...
  private fun handleQuotes(element: String): String =
    element.removeSurrounding(quotechar.toString())
