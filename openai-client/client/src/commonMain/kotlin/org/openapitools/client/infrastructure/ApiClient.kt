...
open class ApiClient(val baseUrl: String, val client: OkHttpClient = defaultClient) {
  ...
  protected inline fun <reified T> requestBody(content: T, mediaType: String?): RequestBody =
    when {
      content is File ->
        content.asRequestBody((mediaType ?: guessContentTypeFromFile(content)).toMediaTypeOrNull())
          ...
          .apply {
            ...
            (content as Map<String, PartConfig<*>>).forEach { (name, part) ->
              if (part.body is File) {
                ...
                addPart(partHeaders.toHeaders(), part.body.asRequestBody(fileMediaType))
              } else {
                val partHeaders =
                  part.headers.toMutableMap() +
                    ("Content-Disposition" to "form-data; name=\"$name\"")
                addPart(partHeaders.toHeaders(), parameterToString(part.body).toRequestBody(null))
              }
            }
          }
          .build()
      ...
      mediaType == XmlMediaType ->
        throw UnsupportedOperationException("xml not currently supported.")
      mediaType == OctetMediaType && content is ByteArray ->
        content.toRequestBody(OctetMediaType.toMediaTypeOrNull())
      // TODO: this should be extended with other serializers
      else ->
        throw UnsupportedOperationException(
          "requestBody currently only supports JSON body, byte body and File body."
        )
    }
  ...
  protected inline fun <reified T : Any?> responseBody(
    ...
    if (T::class.java == File::class.java) {
      // return tempFile
      // Attention: if you are developing an android app that supports API Level 25 and bellow,
      // please check flag supportAndroidApiLevel25AndBelow in
  ...
  protected inline fun <reified I, reified T : Any?> request(
    ...
    val request =
      when (requestConfig.method) {
          RequestMethod.DELETE ->
            Request.Builder().url(url).delete(requestBody(requestConfig.body, contentType))
          RequestMethod.GET -> Request.Builder().url(url)
          RequestMethod.HEAD -> Request.Builder().url(url).head()
          RequestMethod.PATCH ->
            Request.Builder().url(url).patch(requestBody(requestConfig.body, contentType))
          RequestMethod.PUT ->
            Request.Builder().url(url).put(requestBody(requestConfig.body, contentType))
          RequestMethod.POST ->
            Request.Builder().url(url).post(requestBody(requestConfig.body, contentType))
    ...
    return when {
      response.isRedirect -> Redirection(response.code, response.headers.toMultimap())
      response.isInformational ->
        Informational(response.message, response.code, response.headers.toMultimap())
      response.isSuccessful ->
        Success(responseBody(response.body, accept), response.code, response.headers.toMultimap())
