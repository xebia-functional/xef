...
fun Agent.Companion.wikipedia(executor: CommandExecutor = CommandExecutor.DEFAULT): Agent<WikipediaResult> =
  object : Agent<WikipediaResult> {
    override suspend fun context(task: Task, previousSolutions: List<Solution>): AgentContext<WikipediaResult> {
      val logger = KotlinLogging.logger("WikipediaAgent")
