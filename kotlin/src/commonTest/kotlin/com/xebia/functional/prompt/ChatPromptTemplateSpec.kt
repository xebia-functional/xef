...
class ChatPromptTemplateSpec :
  StringSpec({
    "ChatPromptTemplate should fail when not all inputs are passed" {
      either {
        val systemPrompt: PromptTemplate<SystemMessage> =
        ...
        val humanPrompt: PromptTemplate<HumanMessage> =
          PromptTemplate.human(PromptTemplate("{question}", listOf("question")))

        val chatPrompt = ChatPromptTemplate(listOf(systemPrompt, humanPrompt))

        chatPrompt
          .format(mapOf("input_language" to "English", "question" to "What is your name?"))
          .bind()
      } shouldBeLeft
        InvalidInputs(
    ...
    "ChatPromptTemplate format should return the correct formatted type and content of the AI messages" {
      either {
        ...
        val aiPrompt: PromptTemplate<AIMessage> =
          PromptTemplate.ai(PromptTemplate("{answer}", listOf("answer")))

        val otherHumanPrompt: PromptTemplate<HumanMessage> =
          PromptTemplate.human(PromptTemplate("{otherQuestion}", listOf("otherQuestion")))

        val chatPrompt = ChatPromptTemplate(listOf(humanPrompt, aiPrompt, otherHumanPrompt))
    ...
    "ChatPromptTemplate format with chat messages should return the correct role and content of the Chat Messages" {
      either {
        val systemPrompt: PromptTemplate<SystemMessage> =
          PromptTemplate.system(PromptTemplate("{context}", listOf("context")))

        val humanPrompt: PromptTemplate<HumanMessage> =
          PromptTemplate.human(PromptTemplate("{question}", listOf("question")))

        val anakinPrompt: PromptTemplate<ChatMessage> =
          PromptTemplate.chat(PromptTemplate("{firstPhrase}", listOf("firstPhrase")), "Anakin")

        val obiWanPrompt: PromptTemplate<ChatMessage> =
          PromptTemplate.chat(PromptTemplate("{secondPhrase}", listOf("secondPhrase")), "Obi-Wan")

        val chatPrompt =
          ChatPromptTemplate(listOf(systemPrompt, humanPrompt, anakinPrompt, obiWanPrompt))
    ...
    "ChatPromptTemplate formatMessages should return the formatted list of messages" {
      either {
        ...
        val humanPrompt: PromptTemplate<HumanMessage> =
          PromptTemplate.human(PromptTemplate("{text}", listOf("text")))
