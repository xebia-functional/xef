...
class WikipediaClient : AutoCloseable, AutoClose by autoClose() {
    ...
    @Serializable
    data class SearchResult(@SerialName("query") val searchResults: SearchResults)
    @Serializable
    data class SearchResults(@SerialName("search") val searches: List<Search>)
    @Serializable
    data class Search(
        val title: String,
    ...
    @Serializable
    data class SearchByParamResults(val pages: Map<String, Page>)
    @Serializable
    data class Page(
        @SerialName("pageid") val pageId: Int,
        val title: String,
    ...
    suspend fun search(searchData: SearchData): SearchResult {
        return http.get("https://en.wikipedia.org/w/api.php?format=json&action=query&list=search&srsearch=${searchData.search}") {
                contentType(ContentType.Application.Json)
            }.body<SearchResult>()
    }

    suspend fun searchByPageId(searchData: SearchDataByParam): Page {
        return http.get("https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&exintro&explaintext&redirects=1&pageids=${searchData.pageId}") {
            contentType(ContentType.Application.Json)
