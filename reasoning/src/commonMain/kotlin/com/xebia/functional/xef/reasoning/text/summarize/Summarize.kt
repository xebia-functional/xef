...
class Summarize(
  ...
  private suspend fun summarizeChunk(query: String, chunk: String, summaryLength: Int): String {
    ...
    val maxContextLength: Int = model.modelType.maxContextLength - 1000 //magic padding for functions and memory
    ...
    return model.promptMessage(
      ExpertSystem(
        system = "You are an expert information summarizer that is able to provide a summary of a long text in an exact number of words",
        query = """|
  ...
  private fun chunkText(text: String): List<String> {
    val maxTokens = model.modelType.maxContextLength - 2000 //magic padding for functions and memory
  ...
  tailrec suspend fun summarizeLargeText(
    query: String,
    text: String,
    summaryLength: Int
  ): String {
    logger.info { "📚 Summarizing large text of length ${text.length} to approximately $summaryLength words" }
    ...
    val chunkSummaries = chunks.parMap { chunk ->
