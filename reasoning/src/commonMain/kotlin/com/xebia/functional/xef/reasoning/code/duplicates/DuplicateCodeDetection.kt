...
class DuplicateCodeDetection(
  ...
  suspend fun findDuplicateCode(sourceCode: String, similarityThreshold: Double): DuplicateCodeDetectionResult {
    logger.info { "🔍 Finding duplicate code within the source" }
    return callModel<DuplicateCodeDetectionResult>(
      model,
      scope,
      prompt = ExpertSystem(
        system = "You are an expert in duplicate code detection that can find duplicate code within a single source input",
        query = """|
      ...
      logger.info { "🔍 Duplicate code detection result: $it" }
    }
  }
}
