...
class TestSerpApiClient(private val serpApiKey: String? = "SERP_API_KEY") {
  ...
  init {
      if (serpApiKey.isNullOrBlank())
          throw SerpApiClientException(
              HttpStatusCode.Unauthorized,
              SERP_API_KEY_NOT_FOUND
          )
  }
  ...
  suspend fun search(searchData: SearchData): SearchResults {
    ...
    val searchResults = SearchResults(listOf(searchResult1, searchResult2, searchResult3))

    val jsonString = Json.encodeToJsonElement(searchResults)
