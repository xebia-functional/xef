...
class JDBCSQLImpl(
  ...
  override suspend fun AIScope.promptQuery(
    ...
    val tables = selection.flatMap { it.split(",") }.map { it.trim() }
    val ddl = tables.joinToString("\n") { getTableDDL(it) }
    val sql = sql(ddl, prompt).joinToString { it.trim() }
  ...
  private suspend fun documentsForQuery(
    ...
    val splitter = TokenTextSplitter(
      modelType = config.llmModelType,
      chunkSize = config.llmModelType.maxContextLength / 2,
      chunkOverlap = 10
    )
  ...
  override suspend fun AIScope.getInterestingPromptsForDatabase(): List<String> =
    promptMessage(
  ...
  private fun getTableNames(): List<String> {
    ...
    val rs = metaData.getTables(null, null, "%", types)
  ...
  private fun getTableDDL(tableName: String): String {
    ...
    val ddl = StringBuilder("CREATE TABLE $tableName (\n")
    while (rs.next()) {
      val columnName = rs.getString("COLUMN_NAME")
      val dataTypeName = rs.getString("TYPE_NAME")
      val columnSize = rs.getString("COLUMN_SIZE")
      ddl.append("  $columnName $dataTypeName($columnSize),\n")
    }
    ddl.deleteCharAt(ddl.length - 2) // Remove the last comma
    ddl.append(");")
    return ddl.toString()
  }
