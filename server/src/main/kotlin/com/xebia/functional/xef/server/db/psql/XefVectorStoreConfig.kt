...
class XefVectorStoreConfig(
  ...
  companion object {
    ...
    suspend fun load(configNamespace: String, config: Config? = null): XefVectorStoreConfig =
    ...
    suspend fun XefVectorStoreConfig.getVectorStoreService(
      config: Config,
      logger: Logger
    ): VectorStoreService {
      when (this.type) {
