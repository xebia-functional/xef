...
class TokenRepositoryService(
    ...
    fun createToken(
        ...
        return transaction {
            ...
            if (user.organizations.none { it.id == project.orgId }) {
                throw OrganizationsException("User is not part of the organization of the ${project.name} project")
            }

            val newXefToken = XefTokens.new {
                name = data.name
                userId = user.id
    ...
    fun getTokens(
        ...
        return transaction {
            ...
            XefTokens.find { XefTokensTable.userId eq user.id }.mapNotNull { xefToken ->
                userProjects.find { project -> project.id == xefToken.projectId.value }?.let {
                    xefToken.toTokenFullResponse(it)
                }
            }
    ...
    fun getTokensByProject(
        ...
        return transaction {
            ...
            XefTokens.find((XefTokensTable.userId eq user.id) and (XefTokensTable.projectId eq projectId)).map { xefToken ->
                val project = Project.findById(xefToken.projectId) ?: throw ProjectException("Project not found")
    ...
    fun updateToken(
        ...
        return transaction {
            ...
            val xefTokenUpdated = xefToken.apply {
                name = data.name
