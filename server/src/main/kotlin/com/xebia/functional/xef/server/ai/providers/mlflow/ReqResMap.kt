...
object ChatReqResMap: ReqResMap<ChatResponse, OpenAIResponse> {
    ...
    private val keys: Set<String> = setOf("messages", "temperature", "candidate_count", "stop", "max_tokens")

    override fun path(pathProvider: PathProvider, model: String, principal: UserIdPrincipal): String? =
        pathProvider.chatPath(model, principal)

    override fun mapRequest(from: JsonObject): JsonObject =
        JsonObject(from.mapKeys { mappedFields.getOrDefault(it.key, it.key) }.filterKeys { keys.contains(it) })
...
object EmbeddingsReqResMap: ReqResMap<EmbeddingsResponse, OpenAIEmbeddingResponse> {
    ...
    override fun path(pathProvider: PathProvider, model: String, principal: UserIdPrincipal): String? =
        pathProvider.embeddingsPath(model, principal)
