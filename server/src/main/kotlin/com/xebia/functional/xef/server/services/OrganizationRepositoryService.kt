...
class OrganizationRepositoryService(private val logger: KLogger) {
  ...
  fun getOrganization(token: Token, id: Int): OrganizationFullResponse {
    ...
    return transaction {
      ...
      user.organizations.find { it.id.value == id }?.toOrganizationFullResponse()
  ...
  fun updateOrganization(
    ...
    return transaction {
      ...
      val organization =
        Organization.findById(id) ?: throw OrganizationsException("Organization not found")
      ...
      if (data.owner != null) {
        val newOwner = User.findById(data.owner) ?: throw UserException("User not found")
    ...
    transaction {
      ...
      if (organization.ownerId != user.id) {
        throw OrganizationsException(
          "You can't delete the organization. User is not the owner of the organization"
        )
      }
