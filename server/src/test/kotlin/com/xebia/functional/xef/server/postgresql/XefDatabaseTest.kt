...
class XefDatabaseTest {
  ...
  fun crudOrganization() {
    transaction {
      ...
      ownerUser.delete()
      val deletedOrganization =
        Organization.find { OrganizationsTable.name eq newOrganization.name }
  ...
  fun crudXefTokens() {
    transaction {
      ...
      val config =
        ProvidersConfig(openAI = OpenAIConf(token = "testToken", url = "testUrl"), gcp = null)
      XefTokens.new {
        userId = user.id
