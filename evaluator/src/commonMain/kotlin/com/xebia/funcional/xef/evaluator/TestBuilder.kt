...
data class TestsSpec(
    ...
    @SerialName("outputs_description")
    val outputsDescription: List<String>,
    ...
    companion object {
        @JvmSynthetic
        suspend operator fun invoke(description: String, metric: String = "FactualConsistencyMetric", block: suspend TestBuilder.() -> Unit): TestsSpec =
            TestBuilder(description, metric).apply { block() }.build()
    }
}
...
data class TestSpecItem(
    ...
    @SerialName("actual_outputs")
    val outputs: List<String>
) {
    companion object {
        @JvmSynthetic
        suspend operator fun invoke(input: String, block: suspend TestItemBuilder.() -> Unit): TestSpecItem =
            TestItemBuilder(input).apply { block() }.build()
    }
}
