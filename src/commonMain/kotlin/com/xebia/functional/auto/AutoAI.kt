...
class AutoAI(
    ...
    private suspend fun Raise<TaskCompleted>.taskCreationAgent(
        ...
        val prompt = """
            ...
            This result was based on this task description: $taskDescription. These are incomplete tasks: ${
    ...
    private fun printCurrentTasks() {
        for (t in tasksStorage.getTasks()) {
            println("${t.id.id}. ${t.objective.value}")
        }
    }
     ...
     */
    private suspend fun AutoAI.executeAndStoreTask(task: Task): Pair<String, Task> {
        val result = executionAgent(objective = objective, task = task)
        val firstMessage = getFirstMessage(result) ?: throw IllegalStateException("No message returned")
        val cleanedMessage = cleanResultMessage(firstMessage)
        val taskWithResult = task.copy(
            result = cleanedMessage,
            resultId = TaskId(task.id.id)
        )
        resultsStorage.add(
            task = taskWithResult
        )
