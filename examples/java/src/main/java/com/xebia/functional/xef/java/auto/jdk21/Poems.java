...
public class Poems {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        try (PlatformConversation scope = OpenAI.conversation()) {
            var poem1 = scope.prompt(OpenAI.FromEnvironment.DEFAULT_SERIALIZATION, "A short poem about the beauty of nature.", Poem.class).get();
            var poem2 = scope.prompt(OpenAI.FromEnvironment.DEFAULT_SERIALIZATION, "A short poem about the power of technology.", Poem.class).get();
            var poem3 = scope.prompt(OpenAI.FromEnvironment.DEFAULT_SERIALIZATION, "A short poem about the wisdom of artificial intelligence.", Poem.class).get();
