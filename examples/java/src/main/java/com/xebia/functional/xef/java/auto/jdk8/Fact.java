...
public class Fact {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        try (PlatformConversation scope = OpenAI.conversation()) {
            FactClass fact1 = scope.prompt(OpenAI.FromEnvironment.DEFAULT_SERIALIZATION, new Prompt("A fascinating fact about you"), FactClass.class).get();
            FactClass fact2 = scope.prompt(OpenAI.FromEnvironment.DEFAULT_SERIALIZATION, new Prompt("An interesting fact about me"), FactClass.class).get();

            Prompt riddlePrompt = new Prompt(
