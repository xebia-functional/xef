...
class ConfigSpec :
  StringSpec({
    "Chain Config should return the inputs properly" {
      val config = StringMapChain.Config(setOf("name", "age"), setOf("text"))
    ...
    "Chain Config should return the input as a Map" {
      val config = StringMapChain.Config(setOf("input"), setOf("text"))
    ...
    "Chain Config should fail when inputs set doesn't contain all inputKeys" {
      ...
      result shouldBe
        Either.Left(
          InvalidInputs(
            "The provided inputs: {name} do not match with chain's inputs: {name}, {age}"
          )
        )
    }
    ...
    "Chain Config should fail when input is just one and expects more" {
      ...
      val result = config.createInputs("foo")
      result shouldBe
        Either.Left(InvalidInputs("The expected inputs are more than one: {name}, {age}"))
    }
  })
