...
suspend fun LLMChain(
  ...
  object : StringMapChain {
    ...
    ): Either<AIError.Chain.InvalidInputs, Map<String, String>> = either {
    ...
    private fun formatChatOutput(completions: List<Choice>): Map<String, String> =
      config.outputKeys.associateWith { completions.joinToString(", ") { it.message.content } }

    private fun formatCompletionOutput(completions: List<CompletionChoice>): Map<String, String> =
      config.outputKeys.associateWith { completions.joinToString(", ") { it.text } }
  }
