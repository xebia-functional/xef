...
import com.xebia.functional.xef.tracing.Memories
...
//    scope.track(ContextAllowed(contextAllowed))

    return prompt.copy(messages = contextAllowed + historyAllowed + prompt.messages).also {
      scope.track(
        Messages(
          ...
          remainingTokensForContexts = remainingTokensForContexts,
          countTokens = chat.tokensFromMessages(it.messages),
          memories = memories,
        )
      )
    }
