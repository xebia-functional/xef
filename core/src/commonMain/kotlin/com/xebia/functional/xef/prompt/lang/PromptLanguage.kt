...
fun infer(): Nothing = TODO()

fun summarize(text: String): String = infer()

val devProcess = PromptLanguage {
  "DevProcess" {
    "State" { "Target Language" { "JavaScript"() } }
    "WriteTestsFIRST" {
      "Use Riteway({ given, should, actual, expected })" {
        "Define given, should, actual, and expected inline in the `assert` call."()
        "Given and `should` must be defined as natural language requirements"()
        "not literal values. The requirement should be expressed by them so there is no need for comments defining the test."()
      }
      "Tests must be" {
        "Readable"()
    ...
    "/implement - Implement code in the target language from a SudoLang function or natural language description"()
    "/l | lang - Set the target language"()
    "/h | help"()
