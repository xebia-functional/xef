...
object PromptEvaluator {
  ...
  @Serializable
  data class Score(
    val scoreCriteria: List<ScoreCriteria>,
    val reasoning: String
  )

  @Serializable
  data class ScoreCriteria(val name: String, val score: Double)
  ...
  data class ScoreCriteriaConfig(
      ...
      companion object {
          val DEFAULTS: List<ScoreCriteriaConfig> = listOf(
              ScoreCriteriaConfig(
                  "completeness",
                  "Completeness",
  ...
  suspend fun evaluate(
    ...
    scoreConfig: List<ScoreCriteriaConfig> = ScoreCriteriaConfig.DEFAULTS
      ...
      fun printCall(c: ScoreCriteriaConfig): String = "                ${c.functionTitle}():evaluation=${c.evaluation}"
...
${scoreConfig.joinToString("\n") { printReturn(it) }}
    ...
    val map = Json.decodeFromString(MapSerializer(String.serializer(), JsonPrimitive.serializer()), firstMessage)
    val values: List<ScoreCriteria> = map.toList().flatMap {
